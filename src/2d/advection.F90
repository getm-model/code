...
#ifdef STATIC
   REALTYPE,public                    :: cu(E2DFIELD)
   REALTYPE,public                    :: Di(E2DFIELD)
#else
   REALTYPE,public,dimension(:,:),allocatable       :: Di,cu
#endif
...
!-----------------------------------------------------------------------
   ...
   interface
      ...
      subroutine adv_v_split(dt,f,Di,V,DV, &
...
! !INPUT PARAMETERS:
   ...
   REALTYPE, intent(in) :: delxu(E2DFIELD)    ! dx centered on u-transport pt.
   REALTYPE, intent(in) :: delxv(E2DFIELD)    ! length of y-interface
   REALTYPE, intent(in) :: delyu(E2DFIELD)    ! length of u-interface
   REALTYPE, intent(in) :: delyv(E2DFIELD)    ! dy centered on v-transport pt.
   REALTYPE, intent(in) :: area_inv(E2DFIELD) ! inverse of horizontal box area
...
!
! !OUTPUT PARAMETERS:
   REALTYPE, intent(out), optional   :: Dni(E2DFIELD)
!
! !LOCAL VARIABLES:
   ...
   select case (hor_adv)
      ...
      case ((UPSTREAM_SPLIT),(P2),(Superbee),(MUSCL),(P2_PDM),(FCT))
         ...
         select case (adv_split)
            case (0)
               call adv_u_split(dt,f,Di,U,DU,delxu,delyu,area_inv,au,a2,&
                                hor_adv,az,AH)
               call tic(TIM_ADVH)
...
#ifndef SLICE_MODEL
               call adv_v_split(dt,f,Di,V,DV,delxv,delyv,area_inv,av,a2,&
                                hor_adv,az,AH)
               call tic(TIM_ADVH)
               call update_2d_halo(f,f,az,imin,jmin,imax,jmax,D_TAG)
               call wait_halo(D_TAG)
            ...
            case (1)
               call adv_u_split(dt,f,Di,U,DU,delxu,delyu,area_inv,au,a1,&
                                hor_adv,az,AH)
               call tic(TIM_ADVH)
               call update_2d_halo(f,f,az,imin,jmin,imax,jmax,D_TAG)
               call wait_halo(D_TAG)
