...
!-----------------------------------------------------------------------
!BOP
! !IROUTINE:  upstream_2dh_adv - 2D upstream advection 2d
...
!BOC
   ...
   if (rc /= 0) stop 'upstream_2dh_adv: Error allocating memory (cmax)'

   allocate(cmin(I2DFIELD),stat=rc)    ! work array
   if (rc /= 0) stop 'upstream_2dh_adv: Error allocating memory (cmin)'
#endif
#ifdef SLICE_MODEL
