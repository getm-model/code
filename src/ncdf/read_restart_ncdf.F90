...
!  z is required
   ...
   if (runtype .ge. 2)  then
      ...
      status = &
      nf90_get_var(ncid,Uinto_id,Uinto(istart:istop,jstart:jstop),start(1:3),edges(1:3))
      if (status .NE. NF90_NOERR) then
      ...
      status = &
      nf90_get_var(ncid,Vinto_id,Vinto(istart:istop,jstart:jstop),start(1:3),edges(1:3))
      if (status .NE. NF90_NOERR) then
